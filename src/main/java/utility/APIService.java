...
public class APIService {
    ...
    /**
     *
     * @param userId id of the user whose meal is being set
     * @param foodName name of the meal
     * @param calories calories of user's meal
     * @param totalFat total fat of user's meal
     * @param saturatedFat saturated fat of user's meal
     * @param cholesterol cholesterol of user's meal
     * @param sodium sodium of user's meal
     * @param protein protein of user's meal
     * @return id of meal
     * @throws Exception if creation of meal entry was not successful
     */
    public int setMeal(int userId, String foodName, int calories, double totalFat, double saturatedFat, double cholesterol, double sodium, double protein) throws Exception {
        JSONObject json = new JSONObject();
        json.put("userId", userId);
        json.put("foodName", foodName);
        json.put("calories", calories);
        json.put("totalFat", totalFat);
        json.put("saturatedFat", saturatedFat);
        json.put("cholesterol", cholesterol);
        json.put("sodium", sodium);
        json.put("protein", protein);

        // create and send request
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("https://bitewise-api.onrender.com/api/meals")).header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(json.toString())).build();
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());

        // check response
        if (response.statusCode() == 201) {
            JSONObject obj = new JSONObject(response.body());
            return obj.getJSONObject("meal").getInt("id");
        }
        else {
            throw new Exception("Set meal failed");
        }
    }

    /**
     * returns JSON string of a user's meals with given user id
     * @param userId user id
     * @return String, user's meal history
     * @throws Exception if request fails
     */
    public String getMeals(int userId) throws Exception {
        // create and send request
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("https://bitewise-api.onrender.com/api/meals/")).GET().build();
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
        ...
        if (response.statusCode() == 200) {
            ...
            throw new Exception("Get meals failed");
        }
    }

    /**
     * deletes meal with given meal id
     * @param mealId meal id
     * @throws Exception if request fails
     */
    public void deleteMeal(int mealId) throws Exception {
        // create and send request
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("https://bitewise-api.onrender.com/api/meals/" + mealId)).DELETE().build();
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
